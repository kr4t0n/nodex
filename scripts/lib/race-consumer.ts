...
export async function checkRaceConsumer(page:Page):Promise<void>{
 ...
 await page.evaluate(()=>(window as unknown as {nodexRaceFixture:{setEnabled:(value:boolean)=>void}}).nodexRaceFixture.setEnabled(true));await expect(node).toHaveAttribute('data-nx-frame','2');
 // Arm the recorder before playback: protocol round trips after frame zero can
 // consume its entire dwell on a large consumer page. Observe period commits as
 // well as animation frames, retaining the real intermediate-geometry checks.
 type Sample = { frame: number; y: number; period: string };
 await node.evaluate(element => {
   const target = element as HTMLElement & { raceTrace?: Promise<Sample[]> };
   target.raceTrace = new Promise<Sample[]>(resolve => {
     const armed = performance.now(); let started: number | undefined;
     const samples: Sample[] = [];
     const read = () => {
       if (element.getAttribute('data-nx-animated') !== 'true') return;
       started ??= performance.now();
       const bar = element.querySelector<SVGGraphicsElement>('[data-nx-revenue-bar="a"]');
       if (bar) samples.push({ frame: Number(element.getAttribute('data-nx-frame')), y: bar.getBBox().y, period: element.querySelector('[data-nx-period]')?.textContent ?? '' });
     };
     const observer = new MutationObserver(read);
     observer.observe(element, { attributes: true, attributeFilter: ['data-nx-frame'] });
     const tick = () => {
       read();
       if (performance.now() - (started ?? armed) < (started === undefined ? 8000 : 3200)) requestAnimationFrame(tick);
       else { observer.disconnect(); resolve(samples); }
     };
     tick();
   });
 });
 await page.emulateMedia({reducedMotion:'no-preference'});
 const trace = await node.evaluate(element => (element as HTMLElement & { raceTrace: Promise<Sample[]> }).raceTrace);
