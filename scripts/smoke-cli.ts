...
try {
  ...
  assert.equal(manifest.items.filter((item) => item.meta.tier === 'expressive').length, 3);
  ...
  const report = JSON.parse(await cli(project, ['add', 'hairline-line', 'arc-matrix', 'signal-console/endpoint-latency', 'button', '--no-install', '--json'])) as {
  ...
  const second = JSON.parse(await cli(project, ['add', 'hairline-line', 'arc-matrix', '--no-install', '--json'])) as { written: string[] };
  ...
  const linted = await cli(project, ['lint', `${config.paths.components}/hairline-line`, `${config.paths.components}/arc-matrix`, `${config.paths.components}/button`, `${config.paths.components}/_shared`]);
