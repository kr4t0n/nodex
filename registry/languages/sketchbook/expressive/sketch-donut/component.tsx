...
export interface SketchDonutDatum { id: string; label: string; value: number | null; tone?: SketchTone }
export interface SketchDonutProps extends SketchPresentationProps { data: readonly SketchDonutDatum[] }
...
export function SketchDonut({ data, ...props }: SketchDonutProps) {
  const parts = useMemo(() => data.map(part => ({ ...part, paint: part.tone ? paints[part.tone] : Object.values(paints)[sketchSeed(part.id) % 9]! })), [data]);
  return <SketchPolar {...props} data={parts} kind="donut" hatchWidth="var(--nx-sketch-fillWeight)" />;
}
