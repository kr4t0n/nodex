...
**Structure before decoration.** Square surfaces, dark outlines and offset
...
**Color with a role.** Lilac panels, yellow actions, coral badges and mint
selections make the interface visibly colorful. Blue marks progress and adjustable
values; pale blue distinguishes editable fields. Repeat these assignments across
...
| `--nx-fieldFill` | Pale blue fields and their option pickers |
...
| `--nx-badgeFill`, `--nx-badgeText` | Coral solid badges with ink lettering |
...
Yellow, lilac, mint and coral fills take ink text. Keep supporting text dark
enough to read on both paper and lilac. Pale blue fields retain ink values and
...
selection, not success; coral distinguishes a badge, not an error. The shape,
...
**Square construction.** Containers and controls have square corners. The
small checkbox radius supports its native mark; circular controls keep the
geometry needed to communicate their function. Do not turn panels into pills.

**Hard shadows.** Actions, fields, surfaces and popovers each have their own
...
do not compress the type to preserve a desktop arrangement.
...
moves the button toward its offset and reduces the remaining shadow; release
...
- Rounded panels, pill-shaped actions, hairline outlines or decorative tilting.
